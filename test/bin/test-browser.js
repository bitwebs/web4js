...
const BitspaceServer = require('bitspace/server')
const BitswarmServer = require('@web4/bitswarm-web/server')
const webnet = require('webnet')
const http = require('http')
const tapeRun = require('tape-run')
const path = require('path')
const { createWriteStream, createReadStream } = require('fs')
const { randomBytes } = require('@web4/bitweb-crypto')
...
async function main (opts) {
  ...
  try {
    ...
    else console.log('# [bitspace] skip')
    if (opts.swarm) swarm = createBitswarm()
    else console.log('# [bitswarm] skip')
...
function createBitswarm () {
  const port = 4977
  const server = http.createServer((_req, res) => res.end('bitswarm-web'))
  ...
  return new Promise((resolve, reject) => { 
    ...
    server.listen(port, () => {
      ...
      console.log(`# [bitswarm] Listening on ws://localhost:${port}`)
      console.log(`# [bitswarm] → Proxy available on ws://localhost:${port}/proxy`)
      console.log(`# [bitswarm] → Signal available on ws://localhost:${port}/signal`)
      resolve({
        async close () {
          if (closed) return
          closed = true
...
async function createOneSpace (opts = {}) {
  const tmpDir = opts.dir || await tmp.dir({ prefix: 'web4js-bitspace-' + randomBytes(6).toString('hex'), unsafeCleanup: true })
  ...
  server.on('client-open', () => {
    console.log('# [bitspace] client session opened')
  })
  server.on('client-close', () => {
    console.log('# [bitspace] client session closed')
  })
  server.on('error', (error) => {
    console.log(`# [bitspace] error ${error}`)
  })
  webnetServer.on('error', (error) => {
    console.log(`# [bitspace] webnet error ${error}`)
  })
  await Promise.all([
    new Promise((resolve, reject) => {
      webnetServer.once('listening', () => {
        console.log(`# [bitspace] Server listening on ws://localhost:${opts.port}`)
        resolve()
      })
...
async function createManySpaces (numDaemons, opts) {
  ...
  console.log(`# [bitspace] Using dht bootstrap server listening at ${bootstrapHost}`)
